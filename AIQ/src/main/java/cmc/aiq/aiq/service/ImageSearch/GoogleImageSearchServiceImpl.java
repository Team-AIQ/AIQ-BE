...
public class GoogleImageSearchServiceImpl implements GoogleImageSearchService {
    ...
    public String getProductImageUrl(String productName) {
        try {
            ...
            // ★ [수정 1] 쇼핑 검색(shop.json) 대신 이미지 검색(image.json) API를 활용해 보는 건 어떨까?
            URI uri = UriComponentsBuilder.fromHttpUrl("https://openapi.naver.com/v1/search/image.json")
            ...
            log.info("네이버 이미지 API 실제 요청 URL: {}", uri.toString());
            ...
            if (body != null && body.containsKey("items")) {
                ...
                if (items != null && !items.isEmpty()) {
                    // ★ [수정 2] 이미지 검색 API는 결과 필드명이 "image"가 아니라 "link"로 들어와!
            ...
            log.error("네이버 이미지 검색 중 오류 발생 - 제품명: {}, 에러: {}", productName, e.getMessage());
            return "https://placehold.co/600x400?text=No+Image";
        }
    }
}
